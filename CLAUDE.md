...
이 저장소에서 개발 접수, 수정, 시험, PR, 배포 인계 작업을 할 때는
[공통 스킬](.agents/skills/cams-odoo-delivery/SKILL.md)을 먼저 읽고 따른다.
Codex와 Claude는 이 원문과 그 안의 작업 기록 양식을 함께 사용한다.
새 요청은 사용자에게 먼저 알리고 협의한 범위에서 진행하며,
원도영님을 포함한 개발 요청은 Git 큐로 아스트라에게 모은다.
각 AI는 아스트라가 배정한 작업을 수행하고 결과를 아스트라에게 회신한다.
코드와 작업 문서는 같은 작업 브랜치·PR로 관리한다.
현재 사용자의 직접 지시가 과거 문서와 다르면 사용자 지시가 우선한다.
...
작업을 시작하기 전에 `docs/tasks/AGENT_MERGE_WORKFLOW.md`를 읽고 따른다. 브랜치는
`소문자유형/개발명` 형식으로 만들며, 다른 작업이나 미커밋 변경이 있으면 별도 워크트리를 사용한다.
모든 PR에는 같은 브랜치의 `docs/tasks/작업/<소문자유형>/<개발명>.md`가 포함되어야 한다.
`handoff`는 요청·회신 전용 브랜치이며 코드나 작업 문서 원본을 두지 않는다.
