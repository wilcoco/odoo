...
## 0-B. 전제 변경 (2026-09-03 추가) — **공장 가동 전이다**

준비 기간 **약 1개월**, 그리고 **아직 양산 가동 전**이라 이행실적 데이터를 쌓을 수 없다.
이 사실이 개발 우선순위를 바꾼다.

**핵심 판단**: 가동 전 평가에서 평가자가 볼 수 있는 것은
① **관리기준이 수립되어 있는가**(마스터·기준값·주기)
② **가동 전에 실제로 발생하는 이력**(설비 반입·정도검사, 금형 이관·시사출, 계측기 교정,
   작업자 교육, 내부심사, 4M, 문서 개정)
③ 양산 이후를 대비한 **기록 체계가 준비되어 있는가**(빈 양식이 아니라 동작하는 시스템)

따라서 **"실적을 쌓는 기능"보다 "기준을 담는 마스터 + 가동 전 이력"을 먼저** 만든다.
반대로 **매일 기록이 쌓여야 의미가 생기는 기능**(일상점검 실적 집계 등)은
껍데기만 만들어 두면 오히려 "기준은 있고 실적 없음 = 보완(60%)" 판정을 부른다.

> ⚠️ **데모·샘플 데이터로 실적을 채우지 말 것.** 채점 기준에 "허위기재 확인 시 다수미흡(25%)"
> 이 명시돼 있다. 없는 실적은 **없는 채로 두고, 기준·계획·시스템으로 설명**하는 편이 안전하다.

### 가동 전에도 **진짜 이력이 쌓이는** 항목 — 여기부터 실물 데이터를 만든다

| 항목 | SQ | 왜 지금 가능한가 |
|---|---|---|
| **금형 이관·시사출(T/O) 보고서** | 4_5 | 금형 반입·시사출이 지금 진행되는 일 |
| **설비 정도검사·설치 이력** | 3_2 | 신규 설비 설치가 지금 |
| **계측기 대장·교정 필증** | 2_5 | 지금 등록·교정 가능 (크리아 감점 항목) |
| **작업자 교육·자격인증 이력** | 6_5 / SW 1_3 | 채용·배치 전 교육이 지금 |
| **문서 등록·개정 이력** | 1_1·6_4 | 표준류 제정이 지금 |
| **4M 변경 이력** | 6_2 | 설비·금형 도입 자체가 4M |
| **내부심사(SQ 관점) 계획·실적** | 6_4 / SW 5_4 | 평가 전 자체 점검을 시스템에 남긴다 |

### 이에 따른 우선순위 재배열 (1개월)

| 주차 | 작업 | 이유 |
|---|---|---|
| 1주 | **금형 마스터 기준값**(1-1) + **시사출 T/O 보고서**(1-5) | 지금 실제로 발생하는 이력. 기준+실적을 동시에 확보 |
| 2주 | **금형 점검·세척·온도 기준 및 기록 골격**(1-2~1-4) | 기준(주기·상하한)을 마스터에 넣는 것이 본체. 기록은 가동 시 자동으로 쌓임 |
| 3주 | **범용 점검 일지**(2항) — **시트(기준) 정의 중심** | 항목·주기·상하한이 곧 "관리기준 수립" 증빙 |
| 4주 | **안전관리**(3-1) + 내부심사·교육 데이터 정리 | 가동 전에도 실적이 남는 영역 |

**이번 범위에서 제외**(문서·현장 운영으로 대응): 한도견본, 리워크, 개선활동 집계, 정성품질.
배점 10~20점이고 실적 없이도 기준서·게시로 '양호' 확보가 가능하다.
