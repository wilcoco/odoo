...
- (추가 06:5x) #7 이식·재배포 **`65e356c3` SUCCESS**(health 200, 이식 전 닫힘 검사 OK). UAT main 은 관찰 #5·#6·#7·#10·#11·#16(도구)·정책 ①②·#9/#12·#14·결함 #1·#3·#4·#5 정정을 포함. 남은 대기: 사용자 Q1~Q6 답, 테스트 배정 01(결함 #2·기존 시험 환경 의존).
- (추가 06:5x) 기존 `iatf_process_inspection` 11 error 런타임 분리 완료: 모듈 부재 7(spc/risk 5, traceability 2)+단독 경로 고정물 4 → 시험 정정 d75f7cf, 재실행 0/72 error. 제품 결함 아님, 게이트 무변경. 상세는 20260915-01 회신.
- (추가 08:0x) **Q1 사용자 확정**: 안전재고 기준 = 원청 생산계획에 수요가 있는 날 N개(달력일 폐기), 일수는 설정 메뉴. 반영 abdd82d(+자식 선반 재고 0 정정, 테스트 원인 확정 채택). injection_planning 0/246, 반증 1 failed. UAT 이식·재배포 진행. 남은 답: Q2~Q6.
- (추가 08:0x) Q1 반영 `injection_planning` 이식(uat/main 56da676, 닫힘 OK) → **배포 사고 3(개발 실수)**: `UAT_UPDATE_MODULES=injection_planning@56da676` 이 start.py 허용 문자(`[a-zA-Z0-9_,]+`) 위반 → `Invalid UAT_UPDATE_MODULES` 종료 루프, 278359dd CRASHED·502 약 3분(로그 `artifacts/r144_q1_crash_278359dd.log`). 값을 `injection_planning,injection_worksite,production_planning` 으로 고쳐 재배포 **9b08ca13 SUCCESS, health 200**, CRITICAL/Traceback 0(`artifacts/r144_q1_redeploy_9b08ca13.log`, Modules loaded 2회). 코드 문제 아님, 메모리 절차에 함정 기록. UAT 는 Q1 확정본 상태.
