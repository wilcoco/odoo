...
### 7.4 장비 연동 확인 (복제본 리허설에서 먼저 수행, 운영 재기동 후 같은 순서로 반복)

코드가 같아져도 실물 장비는 운영·복제본에서만 붙는다. 개발·UAT 는 실물 PLC·MQTT·센서 연결이 금지·차단돼 있으므로 아래 항목은 원도영이 확인하고 결과(시각·건수·로그 줄)를 §9 증적에 남긴다.

| 항목 | 방법 | 통과 기준 |
|---|---|---|
| PLC 완료 신호 → 단위 MO·시리얼 | 복제본: `fake_plc/injection_api_tester.py` (미러·정본 동일 경로) 로 호기 1대에 1 → 2 → 3 한 사이클. 운영: 재기동 후 실제 사출기의 다음 완료 신호 1건을 기다림 | `/api/plc/request` 200 후 시리얼 1건 발행, `/api/plc/complete` 200 후 생산 실적 1건 생성, 같은 request_id 재전송 시 중복 생성 없음, 로그에 Traceback 없음 |
| 사일로 센서 잔량 | 크론 `PLC SILO 잔량 동기화` 를 수동 1회 실행 (`action_sync_plc_silo_values`) 뒤 사일로 화면 확인 | `public.t_silo` 최신 행 시각이 재기동 이후로 갱신되고 화면의 환산 잔량(kg)이 그 행과 같음. 잔량 9톤 이하 사일로가 있으면 크론 `SILO 저재고 자동발주 점검` 이 통보를 1건만 만들고 열린 발주가 있으면 만들지 않음 |
| 바코드 스캔 → 판정 | 작업장 화면에서 스캐너로 시리얼 1건 스캔 (`/api/injection/barcode/scan`) | 판정 결과가 화면에 표시되고 응답 200, 잘못된 시리얼은 오류 문구로 거부 |
| MQTT 브로커 | `gh_mqtt_setup` 설정 화면의 접속 테스트 1회 | 상태 success, 실패 시 상태 error 와 사유 표시 |
| 원청 수요 연동(Oracle) | `erp_plan_sync` 동기화 1회 (크론 또는 수동) | 오류 없이 종료, 마지막 동기화 시각 갱신 |

리허설에서 하나라도 실패하면 운영 일정을 잡지 않는다. 실패 항목은 인계 큐로 개발에 보내고, 계약(요청·응답 형식)을 고치는 경우 `fake_plc/injection_api_tester.py` 와 그 단위시험을 같은 커밋에서 함께 고친다.
