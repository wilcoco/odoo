...
15. **개발 → 머지 → 배포 4단계** (서버 담당 확정 2026-09-10): ① 작업 단위 브랜치 + **같은 브랜치의
    `docs/tasks/작업/` 에 작업 내용 기록** ② PR → 18.0 (코드·문서 함께 검토) ③ 배포 저장소
    `escon-odoo/odoo_gh iatf_plugins/` 정본 동기화 ④ 운영 서버 반영. **문서 전용 브랜치를 만들지 않는다.**
    통신 큐 `handoff` 브랜치만 예외(머지 안 함). 운영 DB 복제본은 테스트베드 — 절대 운영으로 되돌리지 않는다.
