...
`x_escon_tax_approval_no`를 승인번호로 조회하면 안 됩니다. 예외는 단 하나, 전표를
만들거나 `ref`를 바꿀 때 `ref`가 국세청 24자리 승인번호 형식이고 정본이 비어 있으면
**매입·매출 네 유형 모두** 정본으로 한 번 복사합니다(더존 이관 호환, 18.0.1.5.1부터
매출 포함). 그 뒤로는 `ref`를 바꿔도 정본을 덮어쓰지 않습니다. `ref`는 공급업체의 자체
...
모듈 업그레이드는 `x_escon_tax_approval_no`를 자동 삭제하지 않습니다. 내부 참조 점검은
`승인번호 필드 통합` 위저드 자신의 화면은 제외합니다(제거 확인 문구가 자기참조로
집계되어 제거가 영원히 보류되던 문제, 18.0.1.5.1). 회계 관리자가
