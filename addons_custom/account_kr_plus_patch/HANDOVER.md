...
> 이 문서는 이 모듈의 **소수점 표시 구성요소만** 대상으로 합니다.
> 현재 모듈에는 회계 화면·전표번호·은행계좌 연결 기능도 있으므로 모듈 전체를
> 폐기하거나 uninstall하면 안 됩니다.
...
존재할 수 있습니다. 과거 문서의 `account_kr_plus_patch` 전체 uninstall 절차는
더 이상 유효하지 않습니다. 현재 모듈을 제거하면 회계 화면·전표번호·은행계좌 연결도
함께 제거됩니다.

이관 전 별도 마이그레이션에서 다음을 함께 설계해야 합니다.

1. 기존 `KR Amount` 레코드와 XML ID의 소유권 이전 또는 재사용
2. `kr_amount_digits.py`와 관련 테스트의 K-Guard 편입
3. Plus Patch 매니페스트에서 소수점 데이터·모델만 제거
4. 두 모듈을 같은 배포에서 업그레이드한 뒤 서버 재시작

새 모듈에서 같은 이름의 레코드를 먼저 생성하면 unique 위반으로 설치가 중단됩니다.
운영 데이터베이스를 대상으로 마이그레이션 리허설을 통과하기 전에는 이관하지
않습니다.
